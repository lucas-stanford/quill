 ...
 *
 * **They are read-only.** The plan is the artifact that becomes tickets and the
 * only thing the exit protocol reports on. Making companions editable would
 * mean a sidecar, a revision and a conflict story for each of them, and would
 * blur which document the review is actually about.
 *
...
export async function readCompanion(
  ...
  try {
    ...
    return { ok: true, document: { name, label: companionLabel(name), path, markdown } };
  } catch (err) {
